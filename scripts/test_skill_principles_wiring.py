...
progressive-disclosure INDEX (`--index`), not the full-corpus dump.
"""
...
def test_skills_use_index_command_not_inline_bash():
    """Progressive-disclosure contract: the ALWAYS-injected path is the single `--index` helper
    (progressive disclosure — full bodies pulled on demand via `--show`), and NONE
    of the old hand-rolled inline bash (which twice shipped a silent-failure bug —
    CB1 swallowed the resolver exit, R6 swallowed a cat). All error handling lives
    in Python, so the skill markdown can't reintroduce the class."""
    for s in CHAIN_SKILLS:
        txt = (SK / s / "SKILL.md").read_text()
        assert "principles_context.py" in txt and "--index" in txt, (
            f"{s} does not invoke the --index helper"
        )
        # every skill documents the on-demand expansion path
        assert "--show" in txt, f"{s} does not document the --show expansion path"
        # the always-injected path must NOT be the full-corpus dump
        assert "--emit --project-root" not in txt, (
            f"{s} still injects the full-corpus --emit dump on every run"
        )
...
def test_index_block_hard_errors_end_to_end(tmp_path):
    ...
    (the helper centralizes the failure handling). Happy path exits 0 and emits the
    principle in index form (id + title). Proves the contract end-to-end, not just
    by source-string inspection."""
    # fake plugin root with a working copy of the resolver + a real seeded principle
    ...
    (plugin / "knowledge" / "principles.md").write_text(
        "### P-001. Example Rule\n> the one-line rule.\n\nbody text.\n"
    )
    proj = tmp_path / "proj"
    proj.mkdir()
    ...
    for s in CHAIN_SKILLS:
        ...
        # 2) happy path -> exit 0 and emits the principle in index form
        ok = run(block)
        assert ok.returncode == 0 and "P-001. Example Rule" in ok.stdout, (
            f"{s} happy path broken"
        )
